...
1. Ejecutar `.\dev.bat`.
2. Abrir [http://localhost:8000/health](http://localhost:8000/health) y [http://localhost:5173](http://localhost:5173).

**Criterios de aceptación**

- [x] `ml_ready: true` en `/health`.
- [x] Login como `clinician@medscope.ai` redirige al dashboard sin errores de consola.
...
- [x] Título y secciones del formulario (demographics, vitals, admissions).
- [x] Botón **Generate AI prediction** visible.
- [x] Ya no es página placeholder (Fase 4).
...
- [x] Mensajes de error junto a los campos afectados.
- [x] No se llama a la API (sin navegación a resultado).
- [x] Tras corregir valores, el error desaparece al revalidar.
...
- [x] Navegación a `/evaluation/result`.
- [x] Sin toast/error de API.
- [x] Tiempo de respuesta percibido < 3 s en entorno local (objetivo RNF-001: < 1 s backend).
...
- [x] Gauge circular con porcentaje numérico.
- [x] Badge **LOW / MEDIUM / HIGH RISK** coherente con el score.
- [x] Bloque de recomendación clínica (RiskIndicator) debajo del gauge.
...
- [x] Lista de factores ordenados por importancia.
- [x] Barras rojas hacia la derecha (aumentan riesgo) y teal hacia la izquierda (reducen riesgo).
- [x] Leyenda **Increased risk** / **Decreased risk** visible.
...
1. Abre directamente [http://localhost:5173/evaluation/result](http://localhost:5173/evaluation/result) (sin haber predicho en la sesión).

**Criterios de aceptación**

- [x] Redirección a `/evaluation` (no pantalla en blanco ni error).
...
- [x] Nurse no ve Evaluation en sidebar (o ruta no accesible según configuración actual).
- [x] Clinician y admin sí acceden al flujo completo.
...
- [x] ESLint sin errores (warnings preexistentes aceptables).
...
- [x] Ruff + ESLint OK.
- [x] Backend: **93** passed.
- [x] ML: **80** passed, **1** xfail esperado.
...
**Siguiente fase manual (mismo sprint clínico):** [Phase 05 — Simulación UI](Phase-05-Clinical-Simulation-UI.md) (US-021, T-520–523).
